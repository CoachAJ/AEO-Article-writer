...
{
  "articleMarkdown": "The full blog post in Markdown format. Include a compelling H1 title, multiple H2 sections with bolded direct answers, lists, and actionable content. ${contactName || websiteUrl || bookingUrl || phone ? `End with a '## Contact Us' section. Format URLs as clickable markdown links like [Visit Our Website](url). Include: ${contactName ? `Contact: ${contactName}` : ''} ${websiteUrl ? `Website link: [Visit Our Website](${websiteUrl})` : ''} ${bookingUrl ? `Booking link: [Schedule a Consultation](${bookingUrl})` : ''} ${phone ? `Phone: ${phone}` : ''}` : 'Do NOT include a Contact Us section.'}",
...
${contactName ? `Contact Name/Business: ${contactName}` : ''}
...
app.listen(PORT, () => {
  console.log(`AEO Content Studio running at http://localhost:${PORT}`);
});
